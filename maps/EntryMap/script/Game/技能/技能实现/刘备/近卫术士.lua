...
local M = y3.object.ability[904002]

function M.on_cast_shot(ability, cast)
    -- local owner = ability:get_owner();
    -- local id = ability:kv_load('召唤id','integer')
    -- local time = ability:kv_load('存活时间','number')
    -- local callUnit = y3.unit.create_unit(owner,id,owner:get_point(),owner:get_facing())
    -- callUnit:add_tag('召唤')
    ...
    cast:bindGC(y3.timer.wait(0.3, function (timer)
