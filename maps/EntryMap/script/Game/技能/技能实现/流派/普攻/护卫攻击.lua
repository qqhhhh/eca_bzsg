--这里的数字就是物编里技能的ID
local M = y3.object.ability[134275982]

function M.on_add(ability)
    y3.game:event_on('开始战斗回合',function ()
        local base_atk = ability:kv_load('开战时攻击力点数','integer')
        base_atk = ability:get_owner():get_attr('物理攻击','实际') / base_atk
        local add_hp = ability:kv_load('生命值提升'..ability:get_level(),'integer')
